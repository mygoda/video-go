...
export interface InputSlotSpec {
  ...
  hint?: string;
  /**
   * 本槽只有在该槽也有素材时才成立，缺省表示无依赖。
   * 用于上游把多个语义槽压成一个按位置定序数组的场景：Seedance 的 images[]
   * 第 1 张是首帧、第 2 张是尾帧，"只给尾帧"表达不出来，
   * 提交上去会被当首帧用，产出一条语义相反却"成功"的片子。
   * 被依赖的槽必须声明在本槽之前。
   */
...
export interface CreateTaskRequest {
  ...
  canvas_id?: string;
  /** 单卡重跑时携带：产物原地替换该卡片、旧版本进 history，位置/大小/refs 不变 */
  card_id?: string;
}
...
/**
 * HTTP 层错误码 = TaskErrorCode 全集 + 若干传输层码。
 *
 * 拆成两个类型而不是往 TaskErrorCode 里塞传输码：任务失败分类是产品语义
 * （驱动失败三分类的呈现分支），401/403/404 是传输语义，混在一起前端那几个
 * 分支会被污染。这里写成 `TaskErrorCode | ...` 而不是把五个值再抄一遍，
 * 是为了让「全集」这层关系由类型系统保证，抄漏了编译期就会露出来。
 *
...
export type ApiErrorCode =
  ...
  | 'not_found'
  | 'conflict'
  | 'revision_conflict'   // 画布乐观锁：base_revision 落后，需拉快照后重放
  | 'rate_limited'
  | 'quota_exceeded';     // 存储配额超限

/** 所有非 2xx 响应的响应体形态：{ error: ApiError }。 */
export interface ApiError extends Omit<TaskError, 'code'> {
  code: ApiErrorCode;
}
